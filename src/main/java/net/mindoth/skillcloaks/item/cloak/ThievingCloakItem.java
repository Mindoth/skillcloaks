...
public class ThievingCloakItem extends CurioItem {
    ...
    public void appendHoverText(ItemStack stack, @Nullable World world, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        if (!SkillcloaksCommonConfig.COSMETIC_ONLY.get()) tooltip.add(new TranslationTextComponent("tooltip.skillcloaks.thieving_cloak"));
    ...
    public static void visibilityModifier(LivingEvent.LivingVisibilityEvent event) {
        if ( CuriosApi.getCuriosHelper().findEquippedCurio(SkillcloaksItems.THIEVING_CLOAK.get(), event.getEntityLiving()).isPresent()
                || CuriosApi.getCuriosHelper().findEquippedCurio(SkillcloaksItems.MAX_CLOAK.get(), event.getEntityLiving()).isPresent() ) {
