...
public class WoodcuttingCloakItem extends CurioItem {
    ...
    public void appendHoverText(ItemStack stack, @Nullable World world, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        if (!SkillcloaksCommonConfig.COSMETIC_ONLY.get()) tooltip.add(new TranslationTextComponent("tooltip.skillcloaks.woodcutting_cloak"));
    ...
    private static List<BlockPos> getLogsToBreak(World world, BlockPos pos, List<BlockPos> logsToBreak) {
        ...
        while (aroundLogs.hasNext()) {
            ...
            if (world.getBlockState(logPos).getBlock().is(BlockTags.LOGS)) {
    ...
    public static void onBlockBreak(final BlockEvent.BreakEvent event) {
        ...
        if (!world.isClientSide) {
            if (CuriosApi.getCuriosHelper().findEquippedCurio(SkillcloaksItems.WOODCUTTING_CLOAK.get(), event.getPlayer()).isPresent()
                    || CuriosApi.getCuriosHelper().findEquippedCurio(SkillcloaksItems.MAX_CLOAK.get(), event.getPlayer()).isPresent()) {
                if (!event.getPlayer().isCrouching()) {
                    if (event.getState().getBlock().is(BlockTags.LOGS)) {
                        ...
                        ItemStack axeItem = event.getPlayer().getItemBySlot(EquipmentSlotType.MAINHAND);
                        ...
                        if (axeItem.getItem() instanceof AxeItem && durability >= logs.size()) {
                            ...
                            for (BlockPos leafPos : logs) {
                                ...
                                if (aroundLogs.anyMatch(t -> world.getBlockState(new BlockPos.Mutable(t.getX(), t.getY(), t.getZ())).getBlock().is(BlockTags.LEAVES)
                                        && world.getBlockState(new BlockPos.Mutable(t.getX(), t.getY(), t.getZ())).hasProperty(PERSISTENT)
                                        && !world.getBlockState(new BlockPos.Mutable(t.getX(), t.getY(), t.getZ())).getValue(PERSISTENT))) {
                                    isTree = true;
                                }
                            }
                            if (isTree) {
                                for (BlockPos logPos : logs) {
                                    ...
                                    axeItem.hurtAndBreak(1, event.getPlayer(), (holder) -> holder.broadcastBreakEvent(EquipmentSlotType.MAINHAND));
                                }
                            }
                        }
                        else {
                            world.playSound(null, event.getPlayer().getX(), event.getPlayer().getY() + 1, event.getPlayer().getZ(),
                                    SoundEvents.ITEM_BREAK, SoundCategory.PLAYERS, 1, 0.5f);
                        }
                    }
                }
            }
